...
from soil_utils import predict_soil_metrics
...
async def predict_soil(request: SoilPredictionRequest):
    """Predict soil metrics and recommendations"""
    global SOIL_REQUEST_COUNT
    try:
        ...
        results = predict_soil_metrics(req_dict)
        
        SOIL_REQUEST_COUNT += 1
...
async def monitoring():
    ...
    global REQUEST_COUNT, SOIL_REQUEST_COUNT, PREDICTION_TIMES, START_TIME
    ...
    return {
        ...
        "soil_requests": SOIL_REQUEST_COUNT,
        "requests_per_second": round((REQUEST_COUNT + SOIL_REQUEST_COUNT) / uptime, 2) if uptime > 0 else 0,
        "inference_metrics": {
