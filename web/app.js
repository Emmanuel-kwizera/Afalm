...
const requestsLabelEl = document.getElementById('requests-label');
...
const resFert = document.getElementById('res-fert');
const resNpk = document.getElementById('res-npk');
const resSalinity = document.getElementById('res-salinity');
...
// State
let selectedFiles = [];
let diseaseRequestsCount = 0;
let soilRequestsCount = 0;
...
function updateMetricsDisplay(activeTab) {
    ...
    if (activeTab === 'soil') {
        ...
        if (labelEl) labelEl.textContent = 'Disease Predictions';
        totalRequestsEl.textContent = diseaseRequestsCount.toLocaleString();
    }
}
...
async function updateMonitoring() {
    try {
        ...
        diseaseRequestsCount = data.total_requests || 0;
        soilRequestsCount = data.soil_requests || 0;
        ...
        updateMetricsDisplay(activeTab);
    } catch (error) {
...
async function handleSoilAnalysis(e) {
    e.preventDefault();
    const btn = document.querySelector('button[form="soil-form"]');
    btn.disabled = true;
    btn.textContent = 'Analyzing...';
    soilStatus.classList.add('hidden');
    soilResults.classList.add('hidden');

    const payload = {
        nitrogen: parseFloat(document.getElementById('soil-n').value),
        phosphorous: parseFloat(document.getElementById('soil-p').value),
        potassium: parseFloat(document.getElementById('soil-k').value),
        temperature: parseFloat(document.getElementById('soil-temp').value),
        humidity: parseFloat(document.getElementById('soil-hum').value),
        moisture: parseFloat(document.getElementById('soil-moist').value),
        ph: parseFloat(document.getElementById('soil-ph').value),
        rainfall: parseFloat(document.getElementById('soil-rain').value),
        soil_type: document.getElementById('soil-type').value,
        crop_type: document.getElementById('crop-type').value
    };

    try {
        const response = await fetch(`${API_BASE_URL}/api/v1/predict/soil`, {
            method: 'POST',
        ...
        if (data.success) {
            resCrop.textContent = data.data.crop_recommendation || '--';
            resRisk.textContent = data.data.soil_risk || '--';
            resFert.textContent = data.data.fertilizer_prescription || '--';
            if (data.data.target_n) {
                resNpk.textContent = `${data.data.target_n} : ${data.data.target_p} : ${data.data.target_k}`;
            } else {
                resNpk.textContent = '--';
            }
            ...
            soilStatus.textContent = data.detail || 'Analysis failed. Please try again.';
        }
    } catch (error) {
        console.error('Soil Analysis Error:', error);
        soilStatus.classList.remove('hidden');
        soilStatus.classList.add('error');
        soilStatus.textContent = 'Network error. Please make sure backend is running.';
    } finally {
        btn.disabled = false;
